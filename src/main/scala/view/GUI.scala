...
object GUI extends JFXApp {
  ...
  stage = new PrimaryStage {
    ...
    scene = new Scene(600, 563) {
      ...
      val factoryListItems = new ComboBox(factoryType.getTypeNameList)
      ...
      findBtn.onAction = (e: ActionEvent) => {
        if (!findBtn.getText.isEmpty) {
          ...
          new Alert(AlertType.Information) {
            ...
            contentText = "Найденное значение: " + findValue
          }.showAndWait()
        }
        else
          new Alert(AlertType.Information) {
      ...
      saveBtn.onAction = (e: ActionEvent) => {
        if (defaultType eq "Integer") {
          ...
          new Alert(AlertType.Information) {
            title = "Успех"
            headerText = "Список успешно сохранен"
            contentText = "Список сохранен в файл " + FILE_NAME_TIME
          }.showAndWait()
        }
      }
      ...
      loadBtn.onAction = (e: ActionEvent) => {
        if (defaultType eq "Integer") {
          ...
          new Alert(AlertType.Information) {
            title = "Успех!"
            headerText = "Список успешно загружен!"
            contentText = "Список загружен из файла " + FILE_NAME_TIME
          }.showAndWait()
        }
        outTextField.setText(cycleList.toString)
      }
